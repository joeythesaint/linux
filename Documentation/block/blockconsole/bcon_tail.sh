...
end_of_log() {
	...
	SECTORS=`hdparm -g $DEV|grep sectors| \
		sed 's/.*sectors = \([0-9]*\).*/\1/'`
	...
	while true; do
		...
		dd if=$DEV iflag=direct bs=512 count=1 \
			skip=$MIDDLE 2>/dev/null|head -c50 > $BUF
	...
	LEN=`expr $END - $START`
	dd if=$DEV iflag=direct bs=1M count=$LEN skip=$START >$LOGFILE
	echo $LOGFILE
}
